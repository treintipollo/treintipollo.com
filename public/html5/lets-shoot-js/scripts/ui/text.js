...

{
	...
	class Text
	{
		...
		Update(string, color, centralize = false, alwaysOnTop = false, visible = true, alpha = 1)
		{
			...
			this._dimentions.x = this._text.width + SIDE_PADDING * 2;
			this._dimentions.y = this._text.height;
		...
		Drawbackground()
		{
			...
			ShapeUtils.drawRectangleEx(
				this._background,
				-SIDE_PADDING + LINE_THICKNESS + 0.5,
				0 + LINE_THICKNESS + 0.5,
		...
		GetX()
		{
			return -SIDE_PADDING + LINE_THICKNESS;
		}

		GetY()
		{
			return 0 + LINE_THICKNESS;
		}
