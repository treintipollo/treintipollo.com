function Ship(objectContainer) {
	...
	ArrowKeyHandler.addKeyUpCallback(function(keyCode){
		if(keyCode == ArrowKeyHandler.SPACE)  { 
			var rocket = new Rocket(inst.x, 
									inst.y, 
									{x:inst.x + Random.getRandomArbitary(-75, 75)*Random.getRandomBetweenToValues(1, -1), 
									 y:inst.y + Random.getRandomArbitary(-75, 75)*Random.getRandomBetweenToValues(-1, 1)},
									 {x:inst.target.x, y:inst.target.y},
									 inst.container);
			inst.container.add(rocket, 1);
		}
	});
